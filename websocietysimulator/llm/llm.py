...
class LLMBase:
    ...
    def __call__(self, messages: List[Dict[str, str]], model: Optional[str] = None, temperature: float = 0.0, max_tokens: int = 500, stop_strs: Optional[List[str]] = None, n: int = 1) -> Union[str, List[str]]:
        ...
        Args:
            ...
            messages: 输入消息的列表,每个消息是一个字典,包含角色(role)和内容(content)。这通常用于指示消息的来源(例如用户、系统或其他角色)以及消息的具体文本。
            model: 可选的模型覆盖。这意味着你可以指定一个不同的模型来替代默认使用的模型。
            max_tokens: 响应中的最大令牌数,默认为500。令牌是文本的基本单位,可以是单词、字符或其他元素,这取决于使用的模型。这个参数用于限制生成的文本长度。
            stop_strs: 可选的停止字符串列表。如果生成文本时遇到这些字符串中的任何一个,则停止生成。这可以用于防止模型生成不希望的内容。
            n: 要生成的响应数量,默认为1。如果你想要模型生成多个不同的响应,可以设置这个参数。
...
class InfinigenceLLM(LLMBase):
    ...
    def __call__(self, messages: List[Dict[str, str]], model: Optional[str] = None, temperature: float = 0.0, max_tokens: int = 500, stop_strs: Optional[List[str]] = None, n: int = 1) -> Union[str, List[str]]:
        ...
        Args:
            ...
            n: Number of responses to generate, defaults to 1
            参数：
            messages: 输入消息的列表，每条消息都是一个包含角色和内容的字典
            model: 可选的模型覆盖
            max_tokens: 响应中的最大令牌数，默认为 500
            stop_strs: 可选的停止字符串列表
            n: 要生成的响应数量，默认为 1
